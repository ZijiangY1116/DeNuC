...
def main():
    ...
    parser.add_argument("--consep_folder", required=True)
    ...
    train_images = sorted(glob.glob(os.path.join(args.consep_folder, "Train", "Images", "*.png")))
    test_images = sorted(glob.glob(os.path.join(args.consep_folder, "Test", "Images", "*.png")))
    ...
    for subset, image_path in tqdm.tqdm(jobs):
        ...
        inst_map = sio.loadmat(os.path.join(args.consep_folder, subset, "Labels", f"{name}.mat"))["inst_map"]
        save_tiled_sample(
            image, instance_map_to_centers(inst_map), f"{subset.lower()}_{name}", split,
