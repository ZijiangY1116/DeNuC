...
def main():
    ...
    parser.add_argument("--kumar_folder", required=True)
    ...
    train_images = sorted(glob.glob(os.path.join(args.kumar_folder, "train", "Images", "*.tif")))
    split_map = split_train_samples([os.path.splitext(os.path.basename(p))[0] for p in train_images], args.seed)
    jobs = [("train", p) for p in train_images]
    for subset in ("test_same", "test_diff"):
        jobs.extend((subset, p) for p in sorted(glob.glob(os.path.join(args.kumar_folder, subset, "Images", "*.tif"))))
    ...
    for subset, image_path in tqdm.tqdm(jobs):
        ...
        inst_map = sio.loadmat(os.path.join(args.kumar_folder, subset, "Labels", f"{name}.mat"))["inst_map"]
        save_tiled_sample(
