...
def main():
    ...
    parser.add_argument("--cpm15_folder", required=True)
    ...
    images = sorted(glob.glob(os.path.join(args.cpm15_folder, "Images", "*.png")))
    ...
    for image_path in tqdm.tqdm(images):
        ...
        inst_map = sio.loadmat(os.path.join(args.cpm15_folder, "Labels", f"{name}.mat"))["inst_map"]
        save_tiled_sample(
            image, instance_map_to_centers(inst_map), name, "test", args.output_folder,
            mpp=-1, metadata=metadata, save_preview=args.save_preview
        )
